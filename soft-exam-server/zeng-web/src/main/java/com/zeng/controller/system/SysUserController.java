...
import com.zeng.system.service.ISysPostService;
...
public class SysUserController extends BaseController {
    ...
    public ResponseBean getInfo(@PathVariable(value = "userId", required = false) Long userId) {
        ResponseBean ajax = ResponseBean.success();
        List<SysRole> roles = roleService.selectRoleAll();
        ajax.put("roles", SysUser.isAdmin(userId) ? roles : roles.stream().filter(r -> !r.isAdmin()).collect(Collectors.toList()));
        ajax.put("posts", postService.selectPostAll());
        if (StringUtils.isNotNull(userId)) {
            ajax.put(ResponseBean.DATA_TAG, userService.selectUserById(userId));
    ...
    @PostMapping
    public ResponseBean add(@Validated @RequestBody SysUser user) {
        if (UserConstants.NOT_UNIQUE.equals(userService.checkUserNameUnique(user.getUserName()))) {
            return ResponseBean.error("新增用户'" + user.getUserName() + "'失败，登录账号已存在");
        } else if (StringUtils.isNotEmpty(user.getPhone())
                && UserConstants.NOT_UNIQUE.equals(userService.checkPhoneUnique(user))) {
            return ResponseBean.error("新增用户'" + user.getUserName() + "'失败，手机号码已存在");
        } else if (StringUtils.isNotEmpty(user.getEmail())
                && UserConstants.NOT_UNIQUE.equals(userService.checkEmailUnique(user))) {
            return ResponseBean.error("新增用户'" + user.getUserName() + "'失败，邮箱账号已存在");
        }
    ...
    @PutMapping
    public ResponseBean edit(@Validated @RequestBody SysUser user) {
...
//        }
    ...
    public ResponseBean remove(@PathVariable Long[] userIds) {
    ...
    public ResponseBean authRole(@PathVariable("userId") Long userId) {
        ResponseBean ajax = ResponseBean.success();
    ...
    @PutMapping("/authRole")
    public ResponseBean insertAuthRole(Long userId, Long[] roleIds) {
