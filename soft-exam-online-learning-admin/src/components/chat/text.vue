...
export default {
  ...
  methods: {
    addMessageByClick() {
      ...
      let account = chat.currentUser.admin
        ? this.currentSession.userAccount
        : this.currentSession.friendAccount
      let avatar = chat.currentUser.admin
        ? this.currentSession.userAvatar
        : this.currentSession.friendAvatar
      msgObj.mmsg = this.content
      msgObj.mtype = 1
      msgObj.mfromUser = chat.currentUser.userName
      msgObj.mfromAvatar = chat.currentUser.avatar
      //发送群聊消息
      if (this.currentSession.friendAccount == '群聊') {
        msgObj.mwindow = 1
        this.$store.state.chat.stomp.send(
          '/ws/groupChat',
          {},
        ...
        msgObj.mwindow = 0
        msgObj.mtoUser = account
        msgObj.mtoAvatar = avatar
        chat.stomp.send('/ws/chat', {}, JSON.stringify(msgObj))
        // 提交私聊消息记录
    ...
    beforeAvatarUpload(file) {
      //不给机器人发送图片
      if (this.currentSession.userAccount == '机器人') {
    ...
    imgSuccess(response, file, fileList) {
      ...
      if (this.currentSession.friendAccount == '群聊') {
        ...
        msgObj.mfromUser = this.$store.state.chat.currentUser.userName
        msgObj.mfromAvatar = this.$store.state.chat.currentUser.avatar
        msgObj.mtoUser = this.currentSession.friendAccount
        msgObj.mtoAvatar = this.currentSession.friendAvatar
